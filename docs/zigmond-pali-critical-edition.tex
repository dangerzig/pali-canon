...
%
% *** DEPRECATED ***
% This draft is no longer being actively maintained.
% The current working draft is zigmond-pali-LRE-draft.tex
% (targeting Language Resources and Evaluation)
% *** DEPRECATED ***
...
% Document metadata (commented out for submission)
...
\author[1]{Rev.\ Dan Zigmond}

\address[1]{Independent Scholar, Palo Alto, CA, USA. E-mail: djz@shmonk.com}
...
\subsubsection{Digital Scholarly Editing}

The past three decades have seen intensive theoretical engagement with what constitutes a ``digital scholarly edition'' as distinct from a mere digitization of print. \citet{sahle2016} influentially proposed that a digitized edition is not a digital edition---the latter must exploit the capabilities of the digital medium in ways that could not be replicated in print. His catalog of digital scholarly editions identifies key features including machine-addressable text, explicit encoding of editorial decisions, and the capacity to represent multiple textual states simultaneously. \citet{pierazzo2015} provides a comprehensive theoretical framework, arguing that digital editions fundamentally alter the relationship between editor, text, and reader by making editorial process transparent and enabling multiple reading paths.

The Text Encoding Initiative (TEI) provides the dominant framework for representing textual variance in digital form \citep{tei2023}. TEI's apparatus criticus module enables systematic encoding of variant readings, witness sigla, and editorial interventions. Major projects such as the Homer Multitext \citep{dué2019} and the \emph{Faust} edition \citep{bohnenkamp2019} have demonstrated the scholarly value of this approach for classical and literary texts, while the Nestle-Aland tradition has begun incorporating digital methods for New Testament textual criticism.

Automated collation has emerged as a key methodological concern. \citet{dekker2015} describe CollateX, an algorithm for detecting textual variance across multiple witnesses without requiring pre-alignment. \citet{andrews2013} survey the application of computational phylogenetics to manuscript stemmatology, demonstrating how graph-based models can represent complex transmission histories that resist traditional stemmata. The present work employs similar collation principles, though the relatively stable printed editions of the Pāli Canon present different challenges than manuscript traditions with extensive scribal variation.

\subsubsection{Computational Text Analysis in the Humanities}

Beyond critical editing, computational methods have increasingly been applied to literary and historical analysis. \citet{burrows2002} pioneered stylometric approaches using function word frequencies to attribute authorship, methods subsequently refined by \citet{eder2016} and implemented in tools such as Stylo. \citet{moretti2013} articulated a program of ``distant reading'' that uses quantitative methods to identify patterns across large corpora invisible to traditional close reading. \citet{jockers2013} demonstrated text mining approaches to nineteenth-century fiction, while \citet{underwood2019} applied machine learning to trace genre evolution across centuries.

These methods remain contested. \citet{da2019} critiqued computational literary studies for methodological weaknesses, prompting responses defending the field's empirical foundations \citep{underwood2019response}. The debate highlights the importance of transparency in computational methods and careful attention to what quantitative patterns can and cannot reveal about textual meaning. For religious and historical texts, where questions of authenticity and dating have direct scholarly consequences, computational approaches must be situated within established philological frameworks rather than positioned as replacements for traditional methods.

\subsubsection{Data Standards and Sustainability}

Digital humanities projects increasingly attend to questions of data sustainability and reuse. The FAIR principles---Findable, Accessible, Interoperable, Reusable---articulated by \citet{wilkinson2016} have become a touchstone for research data management. For textual corpora, this entails not only open licensing but also standardized formats, persistent identifiers, and clear documentation of editorial principles. The present project publishes its data in structured JSON format with full provenance tracking, though future work may explore conversion to TEI for broader interoperability with digital humanities infrastructure.

\subsubsection{Computational Linguistics for Sanskrit and Pāli}

Computational approaches to Sanskrit have developed considerably over the past two decades. The Sanskrit Heritage Engine \citep{goyal2012,huet2005} provides morphological analysis and sandhi splitting for Sanskrit, while \citet{hellwig2016} developed neural approaches to Sanskrit word segmentation. More recently, \citet{krishna2018} applied sequence labeling to joint word segmentation and morphological tagging.

Pāli has received less computational attention than Sanskrit, though the languages share many morphological features. \citet{alfter2017} developed a finite-state approach to Pāli morphological analysis, and the Digital Pāli Reader \citep{yuttadhammo2013} provides dictionary lookup with sandhi analysis. The Digital Pāli Dictionary, described in §2.3, represents the most comprehensive computational resource currently available for Pāli morphology.

\subsubsection{Textual Stratification in Early Buddhist Studies}

The question of which portions of the Pāli Canon represent the earliest Buddhist teachings has occupied scholars since the nineteenth century. \citet{rhysdavids1903} and \citet{winternitz1933} offered early assessments based on doctrinal and linguistic criteria. More recently, \citet{wynne2004} argued that certain passages in the Sutta Nipāta preserve very early material, while \citet{analayo2011} has conducted extensive comparative studies between Pāli texts and their Chinese parallels to identify early strata.

Linguistic criteria for dating have been proposed by \citet{norman1983,norman1997}, who identified archaic features in certain verses, and by \citet{vonhinuber1996}, whose \emph{Handbook of Pāli Literature} remains a standard reference for questions of textual history. \citet{collins1992} examined the role of formulaic language in oral transmission, noting that repetitive passages may preserve older material. The computational approaches developed in this project aim to complement these philological methods.

\subsubsection{Earlier Computational Work on the Pāli Canon}
...
\begin{enumerate}
\item \textbf{Comprehensive lemmatization} using the Digital Pāli Dictionary (DPD), achieving 97.7\% coverage across the corpus through a combination of direct lookup, sandhi decomposition, handling of orthographic variants, and specialized patterns for titles, compounds, and metrical forms.
...
Forms found & 124,104 \\
Forms not found & 2,983 \\
Sandhi compounds (DPD) & 42,441 \\
Orthographic variants (-n/-m→-ṃ) & 1,553 \\
Metrical normalizations & 2,133 \\
Title/chapter matches & 23 \\
Apadāna title patterns & 144 \\
Causative absolutive forms & 79 \\
Compound splits & 208 \\
DPPN proper nouns & 21 \\
\midrule
\textbf{Lemmatization coverage} & \textbf{97.7\%} \\
\bottomrule
\end{tabular}
\end{table}
...
The remaining 2,983 word forms (2.3\%) not resolved by the lemmatizer are genuine candidates for future DPD expansion. Analysis reveals that 97\% originate from the Khuddaka Nikāya---particularly the Jātaka, Apadāna, Milindapañha, Netti, and Cūḷaniddesa/Mahāniddesa. These texts contain:

\begin{enumerate}
\item \textbf{Technical terminology}: Hermeneutical terms from Netti/Peṭakopadesa (e.g., \emph{samāropano}, \emph{otaraṇo}, \emph{vevacano})
\item \textbf{Rare verse vocabulary}: Archaic or dialectal forms in metrical passages
\item \textbf{Narrative vocabulary}: Words specific to Jātaka storytelling
\item \textbf{Proper nouns}: Names not catalogued in available reference works
\item \textbf{Complex sandhi compounds}: Novel combinations not in the DPD deconstructor
\end{enumerate}

Only 95 unknown forms appear in the four main nikāyas (DN, MN, SN, AN), indicating that the lemmatizer achieves near-complete coverage for the most frequently studied texts.
...
This work would not have been possible without the foundational contributions of many individuals and institutions. The Pali Text Society has maintained scholarly editions of the Tipiṭaka for over a century, providing the authoritative base text for this edition. The Vipassana Research Institute created and freely shared the Chaṭṭha Saṅgāyana digital edition \citep{vri}. GRETIL \citep{gretil} has long provided open access to digitized Pāli texts. SuttaCentral contributed the Mahāsaṅgīti digital edition and the segmentation system that enables cross-referencing between editions \citep{suttacentral}. The Digital Pāli Dictionary project provided the comprehensive morphological database essential for lemmatization \citep{dpd2026}, and the \emph{Dictionary of Pāli Proper Names} \citep{malalasekera1937} remains an invaluable resource for identifying persons, places, and texts in the canon.

The computational pipeline for this edition was developed collaboratively with Claude (Anthropic), whose assistance in code development, debugging, and methodological refinement substantially accelerated this work.
