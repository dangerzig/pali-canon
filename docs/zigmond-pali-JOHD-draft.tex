...
This approach handles the approximately 42,400 sandhi compounds in the corpus but cannot decompose novel combinations not in the DPD. Such forms are flagged as unresolved.
...
Despite 97.7\% lemmatization coverage, 2,983 unique word forms remain unresolved. Analysis reveals that 97\% of these originate from the Khuddaka Nikāya---particularly technical texts like Netti, Cūḷaniddesa, and narrative works like the Jātaka. These include:

\begin{itemize}
\item Technical terminology from commentarial texts (e.g., \emph{samāropano}, \emph{vevacano})
\item Rare verse vocabulary and archaic forms
\item Proper nouns not catalogued in the DPPN
\item Complex sandhi compounds not in the DPD deconstructor
\end{itemize}

Only 95 unknown forms appear in the four main nikāyas (DN, MN, SN, AN), indicating near-complete coverage for the most studied texts. A supplementary file lists all unresolved forms for potential manual review or future DPD updates.
